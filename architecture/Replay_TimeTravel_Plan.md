...
Temporal supports replay but only in code, not visually. Knotarium already has the durable
...
- **`NodeState`** (`Backend/Knotarium.Core/Domain/NodeState.cs`) persists `Inputs`, `Outputs`,
...
- **Input resolution** in `ExecuteScheduledNodesAsync`
  (`Backend/Knotarium.Features/Execution/WorkflowExecutor.cs:580`) builds a node's inputs from
...
(`Backend/Knotarium.Core/Domain/NodePackageManifest.cs`).
...
- **Mock-side-effects mode** (Phase 3): non-idempotent nodes return their **original output from the
  ...
  debugging. This is the capability that distinguishes Knotarium from code-only Temporal replay.
