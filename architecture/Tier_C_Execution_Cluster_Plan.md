...
Primary drift authority: `Backend/Knotarium.Features/module.yaml` (`slice_rules`) enforced by `Backend/Knotarium.Architecture.Tests/SliceBoundaryTests.cs`
...
|---|------|-------|--------------|
| 1 | `Execution → Notifications` | [WorkflowExecutor.cs:31,48,367](../Backend/Knotarium.Features/Execution/WorkflowExecutor.cs) | Executor holds an optional `FailureAlertQueue?` and calls `.Enqueue(instance.Id)` on the failure path. |
| 2 | `Notifications → Execution` | [ErrorWorkflowWorker.cs:171](../Backend/Knotarium.Features/Notifications/ErrorWorkflowWorker.cs) | Worker resolves Execution's concrete `ErrorWorkflowRunEnqueuer` from DI to start the error-handler run. |
| 3 | `Polling → Execution` | [PollRunEnqueuer.cs:19,71](../Backend/Knotarium.Features/Polling/PollRunEnqueuer.cs) | Holds Execution's `WorkflowExecutionQueue`, calls `.QueueExecution(id)`. |
| 4 | `Schedules → Execution` | [WorkflowEnqueueService.cs:24,123](../Backend/Knotarium.Features/Schedules/WorkflowEnqueueService.cs) | Same: holds `WorkflowExecutionQueue`, calls `.QueueExecution(id)`. |
| 5 | `Execution → Polling` (const, IL-invisible) | [WorkflowExecutor.cs:1838](../Backend/Knotarium.Features/Execution/WorkflowExecutor.cs) | Reads `PollRunEnqueuer.PayloadVariableKey` (`"__pollPayload"`); inlined at compile time so NetArchTest can't see it — not in the baseline, but a real source coupling. |

Note what is **not** an edge: `ActiveWorkflowVersionService` lives in
`Knotarium.Infrastructure.Persistence` (an allowed project dependency), not in the Execution slice —
...
Three Core interfaces + one Core constants holder. All go in `Knotarium.Core/Contracts` (or
...
- **Consumer:** `ErrorWorkflowWorker` resolves `IErrorWorkflowRunEnqueuer` instead of the concrete type; drops `using Knotarium.Features.Execution;`.
...
- `dotnet test Backend/Knotarium.Architecture.Tests` — `No_new_cross_slice_edges` must stay green after each step; `SliceScan.CurrentEdges()` should report the two Nodes edges only when Tier C is done.
