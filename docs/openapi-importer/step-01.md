...
Alle stabilen Contracts und Domain-Records, auf die sich `Features`, `Infrastructure` und `Api` stützen, leben in `KnotGarden.Core`. Kein Code aus anderen Modulen ist nach diesem Schritt nötig — alles hier lässt sich isoliert kompilieren und testen.

---

## Neue Dateien

### `KnotGarden.Core/Domain/OpenApi/`
...
### `KnotGarden.Core/Contracts/OpenApi/`
...
### `KnotGarden.Core/Exceptions/OpenApiParseException.cs`
...
- Nur `KnotGarden.Core` — keine externen NuGet-Pakete nötig.
...
**Projekt:** `KnotGarden.Tests` (bestehend)  
**Datei:** `KnotGarden.Tests/OpenApi/CoreModelTests.cs`
...
- [ ] `dotnet build KnotGarden.Core` ohne Fehler/Warnings
- [ ] `dotnet build KnotGarden.Tests` ohne Fehler (Tests kompilieren)
