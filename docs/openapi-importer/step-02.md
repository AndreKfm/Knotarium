...
`Microsoft.OpenApi` wird als NuGet-Paket in `KnotGarden.Infrastructure` hinzugefügt und dort hinter dem `IOpenApiParser`-Contract aus Step 01 gekapselt. `Features` und `Core` haben keine direkte Abhängigkeit auf `Microsoft.OpenApi`.
...
```xml
<!-- KnotGarden.Infrastructure/KnotGarden.Infrastructure.csproj -->
...
### `KnotGarden.Infrastructure/OpenApi/MicrosoftOpenApiParser.cs`

Implementiert `IOpenApiParser` aus `KnotGarden.Core.Contracts.OpenApi`.
...
**Projekt:** `KnotGarden.Tests`  
**Datei:** `KnotGarden.Tests/OpenApi/ParserAdapterTests.cs`

**Test-Fixtures** (committen als eingebettete Ressourcen unter `KnotGarden.Tests/OpenApi/Fixtures/`):
...
public class ParserAdapterTests
{
    ...
    private static ReadOnlyMemory<byte> LoadFixture(string name)
    {
        ...
        var resourceName = $"KnotGarden.Tests.OpenApi.Fixtures.{name}";
...
- [ ] `KnotGarden.Infrastructure` kompiliert (`dotnet build`)
- [ ] `KnotGarden.Tests` kompiliert
