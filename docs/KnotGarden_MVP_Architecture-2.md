...
// appsettings.json
{
  "Database": {
    ...
    "ConnectionString": "Data Source=KnotGarden.db"
  }
}
```
...
- `INodePackageGenerator` interface lives in `KnotGarden.Core` with the signature `Task<GeneratedPackage> GenerateAsync(GenerationRequest request, CancellationToken ct)`. No implementation in v2.
...
/Backend
  /src
    /KnotGarden.Api                       (Host, Minimal APIs, SSE endpoints, Node Editor endpoints)
    /KnotGarden.Core                      (IDs, INodeExecutor, manifests, ExecutionPlan, IExecutionEventPublisher, INodePackageGenerator placeholder)
    /KnotGarden.Infrastructure            (EF Core + provider abstraction, direct-ADO.NET journal writer, secret resolver, audit, OpenTelemetry)
    /KnotGarden.NodeRuntime               (Collectible ALC loader, capability injection, banned-API analyzer, test sandbox, declarative executor)
    /KnotGarden.Features
  ...
  /tests
    ...
    /KnotGarden.NodeRuntime.Tests         (ALC unload, capability isolation, analyzer rules)
