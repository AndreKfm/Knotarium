...
## Invariant Alignment
* **Layer Decoupling (F4 Split):** `KnotGarden.Api` hosts the thin polling service (`SchedulingWorker`), which delegates evaluation and database writes to `ScheduleEvaluationService` inside `KnotGarden.Features`.
...
### 1. Register Hosted Service in [Program.cs](file:///d:/Private/Source/AknSideProjects/Automate/Backend/KnotGarden.Api/Program.cs) [MODIFY]
```csharp
builder.Services.AddHostedService<SchedulingWorker>();
```

### 2. Implement thin Hosted Service in [SchedulingWorker.cs](file:///d:/Private/Source/AknSideProjects/Automate/Backend/KnotGarden.Api/Services/SchedulingWorker.cs) [NEW]
...
### 3. Implement Business Logic in [ScheduleEvaluationService.cs](file:///d:/Private/Source/AknSideProjects/Automate/Backend/KnotGarden.Features/Schedules/ScheduleEvaluationService.cs) [NEW]
