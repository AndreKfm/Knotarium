...
* **Token Binding**: The API controller binds the correlation token from the **`X-Knotarium-Token` header or request body**, completely avoiding query-string security anti-patterns.
...
### 2. Implement Controller POST Endpoint in [ExecutionsController.cs](file:///d:/Private/Source/AknSideProjects/Automate/Backend/Knotarium.Api/Controllers/ExecutionsController.cs) [MODIFY]
```csharp
...
public async Task<IActionResult> ResumeExecutionAsync(
    ...
    [FromHeader(Name = "X-Knotarium-Token")] string? headerToken, // Secure header binding
...
### 3. Implement Transactional Resume in [WorkflowExecutor.cs](file:///d:/Private/Source/AknSideProjects/Automate/Backend/Knotarium.Features/Execution/WorkflowExecutor.cs) [MODIFY]
...
* Write integration tests in `ResumeIntegrationTests.cs` verifying:
  ...
  * **Secure Token Extraction**: Mock requests with X-Knotarium-Token headers and request body. Assert both bind correctly, and query-string parameter resume requests are rejected.
