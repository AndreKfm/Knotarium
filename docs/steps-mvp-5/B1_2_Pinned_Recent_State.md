...
```
* Configure Zustand's middleware (`persist`) to automatically serialize and load the `pinnedNodeIds` and `recentNodeIds` from/to `localStorage` under the key `knotgarden:canvas-palette`.
