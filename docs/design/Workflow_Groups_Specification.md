...
This document defines the architecture, data models, persistence mechanics, and user interactions for implementing custom, group-centric visual workspace management in Knot Garden.
...
```csharp
// Backend domain models (KnotGarden.Core/Domain)
...
1. Modify [Backend/KnotGarden.Core/Domain/WorkflowDefinition.cs](Backend/KnotGarden.Core/Domain/WorkflowDefinition.cs). Inject `WorkflowMetadata? Metadata = null` into the constructor and JSON serialization contracts.
...
2. Open [Backend/KnotGarden.Infrastructure/Persistence/FileWorkflowStore.cs](Backend/KnotGarden.Infrastructure/Persistence/FileWorkflowStore.cs):
...
1. Route endpoints in [Backend/KnotGarden.Api/Program.cs](Backend/KnotGarden.Api/Program.cs):
