# Knotarium MVP — Baseline Architecture
...
The executor depends on `IExecutionEventPublisher` (defined in `Knotarium.Core`); the SSE-specific implementation lives in `Knotarium.Api`. The executor has no knowledge of the transport.
...
| Scheduled triggers | Deferred. When added, will use a `PeriodicTimer`-based scheduler in `Knotarium.Infrastructure`. |
...
/Backend
  /src
    /Knotarium.Api              (Host, Minimal APIs, SSE endpoints)
    /Knotarium.Core             (Strongly-typed IDs, INodeTask, ExecutionPlan, IExecutionEventPublisher, ISecretResolver)
    /Knotarium.Infrastructure   (EF Core SQLite context, Plugin Loader, Secret resolver impl)
    /Knotarium.Features         (Vertical Slices)
