# KnotGarden — Architecture v5.3 (Path B & UI Ergonomics)
...
To ensure clean observability, KnotGarden maintains a strict separation between run-level status, persisted execution node state, and runtime node execution result status.
...
* **Invariant 4.2 (Layer Decoupling):**
  * **`KnotGarden.Api` (Host):** Registers the `IHostedService` background worker thread (`SchedulingWorker`).
  * **`KnotGarden.Infrastructure` (Persistence):** Owns the `ScheduleFires` database schemas and schedule rule persistence.
  * **`KnotGarden.Features` (Features):** The trigger-to-instance policy and enqueuing execution runs.
