...
| --- | --- |
| `ParameterDef` / `ParameterKind` | `ParameterDefinition` record — [NodePackageManifest.cs:8](Backend/Knotarium.Core/Domain/NodePackageManifest.cs#L8). String `Type` discriminator (`"string"`, `"enum"`, `"credentialRef"`, …), not an enum. Add `"dynamicOptions"` / `"resourceLocator"`, plus a `Multiple` flag (default false) for multi-select. Extending the record needs an additive optional field so existing manifests keep deserializing — see the `[JsonConstructor]` at [NodePackageManifest.cs:71](Backend/Knotarium.Core/Domain/NodePackageManifest.cs#L71) for the same pattern used for `Inputs`. |
| `IConnection` / resolved credential | `ServerConfigInfo` + `CredentialRef` → `ICredentialAccessor` / `ISecretResolver` ([CredentialAccessor.cs](Backend/Knotarium.Infrastructure/Persistence/CredentialAccessor.cs)). `IServerConfigStore` holds BaseUrl + CredentialRef. |
| Options endpoint | New minimal-API endpoint, mirror [InlineCodeTestEndpoint.cs](Backend/Knotarium.Api/InlineCodeTestEndpoint.cs) (`Map…Endpoint(this WebApplication)` extension, DI via handler params, returns `Results.Ok`). |
...
**Backend, `Knotarium.Features` (new `Options/` folder) + `Knotarium.Core/Contracts`.**
...
- First concrete loader (one real integration, named e.g. `<integration>.<resource>`): resolves the
  ...
  [HttpRequestNodeTask.cs](Backend/Knotarium.Features/Nodes/HttpRequestNodeTask.cs)).
...
**`Knotarium.Api`, new `OptionsEndpoint.cs`.**
...
**Backend `Knotarium.Features`, shared helper.**
