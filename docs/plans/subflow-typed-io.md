...
### Domain model — `Backend\Knotarium.Core\Domain\`
...
### Manifest system — `Backend\Knotarium.Core\Domain\NodePackageManifest.cs`
...
- Endpoints in `Backend\Knotarium.Api\Program.cs`: CRUD `/api/workflows` (630-738), validate (844),
...
## Critical files
- `Backend\Knotarium.Features\Compiler\WorkflowCompiler.cs`
- `Backend\Knotarium.Core\Domain\WorkflowDefinition.cs`
- `Backend\Knotarium.Features\Compiler\InMemoryNodePackageManifestProvider.cs`
- `Backend\Knotarium.Core\Contracts\NodeExecutionContext.cs`
- `Backend\Knotarium.Features\Nodes\StartNodeTask.cs`, `EndNodeTask.cs`
