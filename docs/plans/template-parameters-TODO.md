...
## Shape
Extend `TemplateManifest` (`Backend/KnotGarden.Api/Services/Templates/TemplateModel.cs`) with:
...
- Substitution reuses the existing recursive node-property walk in
  `Backend/KnotGarden.Api/Services/WorkflowPortability/CredentialSlotModule.cs` — add a **third
