...
A full-fidelity **backup/restore** for a Knotarium instance — distinct from `.kgbundle` export (see
...
| Where | What | Notes |
|---|---|---|
| **Database** (`AppDbContext`) | workflow definitions + **all versions**, active-version pointers, **credentials (encrypted)**, node packages, schedules, polling triggers/cursors, notification channels, app settings (error-workflow, …), execution instances + journals, dead-letter, server configs, workflow groups | SQLite by default (`Knotarium.db` + `-wal`/`-shm`), **pluggable to Postgres** (`Database:Provider`) |
| **File workflow store** | draft workflow definitions | `%APPDATA%/Knotarium/workflows/` — **on disk, not in the DB**; may differ from the DB's published versions |
| **Config / env (NOT backed up as data)** | credential **encryption key** (`Security:Credentials:EncryptionKeyBase64` / `Knotarium_CREDENTIAL_ENCRYPTION_KEY_BASE64`), DB connection string, trusted signing keys | Credentials in the DB are AES-GCM ciphertext under this key |
...
Name: `knotarium-backup-<yyyymmdd-hhmmss>.kgbak`.
...
New `Backend/Knotarium.Api/Services/Backup/` (mirrors `Services/Bundles/`).
