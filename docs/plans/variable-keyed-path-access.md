...
- **Reads** never navigate into it. `$variables.foo.bar` is treated as a single variable literally named
  `"foo.bar"` — [ExpressionEvaluator.cs:551](Backend/Knotarium.NodeRuntime/ExpressionEvaluator.cs#L551).
- **Writes** only set a whole value under a flat name — [SetVariableNodeTask.cs:11-16](Backend/Knotarium.Features/Nodes/SetVariableNodeTask.cs#L11).
- `NavigateJson` (used for **node outputs**) supports dotted paths but **only integer** bracket indexing,
  not string keys — [ExpressionEvaluator.cs:642](Backend/Knotarium.NodeRuntime/ExpressionEvaluator.cs#L642).
...
`Backend/Knotarium.NodeRuntime/VariablePath.cs` (parser is pure, unit-testable, shared by read & write).
...
1. **String-key bracket support in `NavigateJson`** ([:653-662](Backend/Knotarium.NodeRuntime/ExpressionEvaluator.cs#L653)):
...
2. **Navigate into variables** ([:551-555](Backend/Knotarium.NodeRuntime/ExpressionEvaluator.cs#L551)):
   ...
   result (mirrors how `$node.…output.…` already works at [:539-543](Backend/Knotarium.NodeRuntime/ExpressionEvaluator.cs#L539)).
...
`SetVariableNodeTask` ([:11-16](Backend/Knotarium.Features/Nodes/SetVariableNodeTask.cs#L11)) currently does
