...
Generate a complete, valid KnotGarden workflow (nodes + edges + configured properties) from a
...
| Concept | This repo |
| --- | --- |
| Node catalog (LLM input) | `InMemoryNodePackageManifestProvider` (26 built-ins) behind `DbNodePackageManifestProvider` (built-ins → binary packages → DB). Project, don't re-model. [InMemoryNodePackageManifestProvider.cs](Backend/KnotGarden.Features/Compiler/InMemoryNodePackageManifestProvider.cs) |
| Manifest field schema | `ParameterDefinition` (`Name`/`Type`/`Required`/`Values`/`Description`), `OutputDefinition`. [NodePackageManifest.cs](Backend/KnotGarden.Core/Domain/NodePackageManifest.cs) |
| Target representation | `WorkflowDefinition` (`Nodes`/`Edges`/`Metadata`), `NodeDefinition(Id,Type,Properties)`, `EdgeDefinition(Id,From,Output,To,Input)`. [WorkflowDefinition.cs](Backend/KnotGarden.Core/Domain/WorkflowDefinition.cs) |
| Validator (repair signal) | `WorkflowCompiler.CompileAsync` → `ERR_INVALID_NODE_TYPE`, `ERR_MISSING_REQUIRED_PARAMETER`, `ERR_INVALID_SOCKET_MAPPING`, `ERR_CYCLE_DETECTED`, … In-process, no round-trip. [WorkflowCompiler.cs](Backend/KnotGarden.Features/Compiler/WorkflowCompiler.cs) |
| Persist path (save) | `POST /api/workflows` → compile → `IWorkflowStore.UpsertAsync`. Reused unchanged by the canvas save. [Program.cs:1047](Backend/KnotGarden.Api/Program.cs#L1047) |
| Outbound LLM call | `IHttpClientFactory.CreateClient("HttpNode")` — wrapped in `HttpEgressPolicyHandler` (SSRF/allowlist). `api.anthropic.com` must be allowlisted. [HttpRequestNodeTask.cs](Backend/KnotGarden.Features/Nodes/HttpRequestNodeTask.cs), [HttpEgressPolicyHandler.cs](Backend/KnotGarden.Infrastructure/Security/HttpEgressPolicyHandler.cs) |
| API key | `ISecretResolver` — `env:ANTHROPIC_API_KEY` or encrypted credential store. [ISecretResolver.cs](Backend/KnotGarden.Core/Contracts/ISecretResolver.cs), [CredentialAccessor.cs](Backend/KnotGarden.Infrastructure/Persistence/CredentialAccessor.cs) |
| Background job | `Channel<T>` queue + `BackgroundService` worker + scoped DI per item. Mirror `FailureAlertQueue`/`FailureAlertWorker`. [FailureAlertWorker.cs](Backend/KnotGarden.Features/Notifications/FailureAlertWorker.cs) |
| Unbound credentials | `CredentialSlotModule` (`slot:<key>` extract/rebind/find-unbound) + `CredentialSlotBinding.tsx`. Reused as-is. [CredentialSlotModule.cs](Backend/KnotGarden.Api/Services/WorkflowPortability/CredentialSlotModule.cs) |
...
**Backend, `KnotGarden.Features` (new `Ai/` folder).**
...
**Backend, `KnotGarden.Features/Ai` + `KnotGarden.Core/Contracts`.**
...
**Backend, `KnotGarden.Features/Ai`.**
...
**Backend, `KnotGarden.Features/Ai` (layout helper may live wherever dagre is callable server-side; otherwise defer geometry to the frontend Tidy on load — see note).**
...
**Backend, `KnotGarden.Features/Ai` + `Program.cs` DI.**
...
**Backend, `KnotGarden.Api` (new `AiGenerationEndpoint.cs`, mirror `InlineCodeTestEndpoint` extension style).**
