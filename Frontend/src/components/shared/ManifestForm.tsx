...
export function FieldDropWrapper({ workflowId, value, onChange, children }: FieldDropWrapperProps) {
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const tokenDataRaw = e.dataTransfer.getData('application/knotgarden-variable-token');
    if (tokenDataRaw) {
      try {
    ...
    const outputDataRaw = e.dataTransfer.getData('application/knotgarden-node-output');
  ...
  const handleTokenDragStart = (e: React.DragEvent<HTMLDivElement>, v: typeof refVar) => {
    ...
    e.dataTransfer.setData('application/knotgarden-variable-token', JSON.stringify(tokenData));
