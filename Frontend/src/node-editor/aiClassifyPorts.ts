// Port derivation for the AI Classify node. Its output branches come from the node's own
...
//
// The parsing rules MUST mirror AiClassifyNodeTask.ParseCategories on the backend
...
export const AI_CLASSIFY_OTHERWISE_PORT = 'otherwise';

/** Parse the raw 'categories' property into the ordered, deduplicated label list. */
export function parseAiClassifyCategories(raw: unknown): string[] {
...
export function aiClassifyOutputHandles(properties: Record<string, unknown> | undefined): string[] {
  return [...parseAiClassifyCategories(properties?.categories), AI_CLASSIFY_OTHERWISE_PORT];
}
