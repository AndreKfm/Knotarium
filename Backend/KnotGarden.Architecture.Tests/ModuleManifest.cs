...
internal sealed class ModuleManifest
{
    ...
    /// <summary>Walk up from the test binary to the Backend folder (the one holding KnotGarden.slnx).</summary>
    public static string BackendRoot()
    {
        var dir = new DirectoryInfo(AppContext.BaseDirectory);
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "KnotGarden.slnx")))
            dir = dir.Parent;
        if (dir is null) throw new InvalidOperationException("Could not locate Backend root (KnotGarden.slnx).");
        return dir.FullName;
    }
