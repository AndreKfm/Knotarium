...
public sealed class ServerConfigApiTests : IClassFixture<KnotGardenApiFactory>, IDisposable
{
    ...
    public ServerConfigApiTests(KnotGardenApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotgarden-servercfg-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotgarden-servercfg-wf-{Guid.NewGuid():N}");
