...
public sealed class OptionsEndpointTests : IClassFixture<KnotGardenApiFactory>, IDisposable
{
    ...
    public OptionsEndpointTests(KnotGardenApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotgarden-options-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotgarden-options-wf-{Guid.NewGuid():N}");
