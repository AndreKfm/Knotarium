...
public sealed class ExternalSystemsEndpointTests : IClassFixture<KnotGardenApiFactory>, IDisposable
{
    ...
    public ExternalSystemsEndpointTests(KnotGardenApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotgarden-extsys-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotgarden-extsys-wf-{Guid.NewGuid():N}");
        ...
        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                services.AddScoped<KnotGarden.Core.Contracts.IExecutionJournalWriter>(_ => new SqliteExecutionJournalWriter(connectionString));
