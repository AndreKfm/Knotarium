...
public sealed class OpenApiApiTests : IClassFixture<KnotGardenApiFactory>, IDisposable
{
    ...
    public OpenApiApiTests(KnotGardenApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotgarden-openapi-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotgarden-openapi-wf-{Guid.NewGuid():N}");
        ...
        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                var writerDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(KnotGarden.Core.Contracts.IExecutionJournalWriter));
                if (writerDescriptor != null) services.Remove(writerDescriptor);
                services.AddScoped<KnotGarden.Core.Contracts.IExecutionJournalWriter>(_ => new KnotGarden.Infrastructure.Persistence.SqliteExecutionJournalWriter(connectionString));
                ...
                var storeDescriptor = services.SingleOrDefault(d => d.ServiceType == typeof(KnotGarden.Core.Contracts.IWorkflowStore));
    ...
    private static byte[] LoadFixture(string name)
    {
        ...
        var resourceName = $"KnotGarden.Tests.Api.OpenApi.Fixtures.{name}";
