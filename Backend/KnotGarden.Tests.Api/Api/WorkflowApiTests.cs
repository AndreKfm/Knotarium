...
public class WorkflowApiTests : IClassFixture<KnotGardenApiFactory>, IDisposable
{
    ...
    public WorkflowApiTests(KnotGardenApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotgarden-api-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotgarden-api-wftests-{Guid.NewGuid():N}");
    ...
    public async Task ResumeExecution_WithHeaderToken_ConsumesTokenAndQueuesWorkItem()
    {
        ...
        request.Headers.TryAddWithoutValidation("X-KnotGarden-Token", rawToken);
