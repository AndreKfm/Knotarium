...
public static class HostPluginServiceCollectionExtensions
{
    public static IServiceCollection AddHostPlugins(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        var hostPlugins = Knotarium.NodeRuntime.HostPluginLoader.Load(pluginsPath, configuration, pluginLoggerFactory);
        services.AddSingleton(hostPlugins);
        if (hostPlugins.SignalProvider != null)
        {
            services.AddSingleton<Knotarium.Core.Contracts.IExternalSignalProvider>(hostPlugins.SignalProvider);
        }
        if (hostPlugins.SignalAdmin != null)
        {
            services.AddSingleton<Knotarium.Core.Contracts.IExternalSignalAdmin>(hostPlugins.SignalAdmin);
        }
        foreach (var optionsLoader in hostPlugins.OptionsLoaders)
        {
            services.AddSingleton<Knotarium.Core.Contracts.Options.IOptionsLoader>(optionsLoader);
        }
        if (hostPlugins.BackgroundLoops.Count > 0)
        {
            services.AddHostedService<Knotarium.NodeRuntime.HostPluginBackgroundRunner>();
        }
