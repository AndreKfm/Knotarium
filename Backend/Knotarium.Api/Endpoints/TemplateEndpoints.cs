...
public static class TemplateEndpoints
{
    public static void MapTemplateEndpoints(this WebApplication app)
    {
        app.MapPost("/api/templates/export", async (
            Knotarium.Features.Templates.TemplateExportRequest request,
            Knotarium.Features.Templates.TemplateExportService exportService,
        ...
        app.MapPost("/api/templates/inspect", async (
            HttpRequest request,
            Knotarium.Features.Templates.TemplateInspectService inspectService,
            CancellationToken cancellationToken) =>
        {
        ...
        app.MapPost("/api/templates/install", async (
            HttpRequest request,
            Knotarium.Features.Templates.TemplateInstallService installService,
            CancellationToken cancellationToken) =>
        {
            var (bytes, error) = await ReadTemplateUploadAsync(request, cancellationToken);
            if (error is not null)
        ...
        app.MapPost("/api/templates/payload", async (
            HttpRequest request,
            Knotarium.Features.Templates.TemplatePayloadService payloadService,
            CancellationToken cancellationToken) =>
        {
            var (bytes, error) = await ReadTemplateUploadAsync(request, cancellationToken);
            if (error is not null)
        ...
        app.MapGet("/api/templates/gallery", async (
            Knotarium.Features.Templates.BuiltInTemplateGallery gallery,
            CancellationToken cancellationToken) =>
        {
        ...
        app.MapPost("/api/templates/library/save", async (
            Knotarium.Features.Templates.TemplateExportRequest request,
            Knotarium.Features.Templates.UserTemplateLibrary library,
            CancellationToken cancellationToken) =>
        {
            if (request is null || string.IsNullOrWhiteSpace(request.WorkflowId))
            {
