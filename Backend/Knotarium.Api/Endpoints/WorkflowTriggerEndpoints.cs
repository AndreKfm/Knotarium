...
public static class WorkflowTriggerEndpoints
{
    ...
    {
        app.MapPost("/api/workflows/{id}/enabled", async (string id, SetEnabledRequest request, IWorkflowStore workflowStore, AppDbContext db, Knotarium.Api.Services.ExternalSignalTriggerRegistry externalSignalRegistry) =>
        {
        ...
        app.MapPost("/api/workflows/{id}/simulate-signal", async (
            ...
            Knotarium.Core.Contracts.IExternalSignalRunEnqueuer enqueuer,
            CancellationToken cancellationToken) =>
        {
            ...
            var kind = string.Equals(request.Kind, "event", StringComparison.OrdinalIgnoreCase)
                ? Knotarium.Core.Contracts.ExternalSignalKind.Event
                : Knotarium.Core.Contracts.ExternalSignalKind.Action;

            // Resolve the pin's downstream entry node(s) from the compiled signal triggers (the imperative bridge).
            var triggers = Knotarium.Core.Reactive.ReactiveRuleCompiler.CompileSignalTriggers(runtimeWorkflow);
            var matched = triggers.Where(trigger =>
            {
                if (trigger.Kind != kind) return false;
                var (baseType, _) = Knotarium.Core.Reactive.ReactiveEventPhase.Parse(trigger.SignalType);
            ...
            var envelope = new Knotarium.Core.Contracts.InboundEnvelope(
                SystemId: targetId,
                TargetId: targetId,
                Host: "simulated",
                Kind: kind,
                ...
                ChannelId: null,
                Active: kind == Knotarium.Core.Contracts.ExternalSignalKind.Event ? true : null,
                CorrelationKey: $"sim-{Guid.NewGuid():N}",
                Payload: payload,
                Timestamp: DateTimeOffset.UtcNow);

            var provenance = new Knotarium.Core.Contracts.DeviceEventProvenance(
                matched[0].SourceNodeId,
                Knotarium.Api.Services.ExternalSignalTriggerRegistry.FormatFiredPinLabel(matched[0].Kind, matched[0].SignalType));
