...
public sealed class AiGenerationEndpointTests : IClassFixture<KnotariumApiFactory>, IDisposable
{
    ...
    public AiGenerationEndpointTests(KnotariumApiFactory factory)
    {
        _databasePath = Path.Combine(Path.GetTempPath(), $"knotarium-ai-tests-{Guid.NewGuid():N}.db");
        _tempWorkflowStoreFolder = Path.Combine(Path.GetTempPath(), $"knotarium-ai-wf-{Guid.NewGuid():N}");
        ...
        _factory = factory.WithWebHostBuilder(builder =>
        {
            builder.ConfigureServices(services =>
            {
                ...
                var store = services.SingleOrDefault(d => d.ServiceType == typeof(Knotarium.Core.Contracts.IWorkflowStore));
