...
builder.Services.AddSingleton<KnotGarden.NodeRuntime.INodeExecutorRegistry, KnotGarden.NodeRuntime.NodeExecutorRegistry>();
builder.Services.AddHostedService<KnotGarden.NodeRuntime.NodePackageWatcher>();
...
builder.Services.AddScoped<KnotGarden.Core.Contracts.IWorkflowTriggerSynchronizer>(
...
builder.Services.AddSingleton<KnotGarden.Api.Services.ExternalSignalTriggerRegistry>(sp =>
    new KnotGarden.Api.Services.ExternalSignalTriggerRegistry(
        sp.GetRequiredService<Microsoft.Extensions.DependencyInjection.IServiceScopeFactory>(),
        sp.GetRequiredService<ILogger<KnotGarden.Api.Services.ExternalSignalTriggerRegistry>>(),
        sp.GetRequiredService<KnotGarden.Api.Services.RuntimeArmingState>(),
        sp.GetService<KnotGarden.Core.Contracts.IExternalSignalProvider>()));
builder.Services.AddHostedService<KnotGarden.Api.Services.ExternalSignalStartupReconciler>();
...
builder.Services.AddScoped<KnotGarden.Api.Services.WorkflowLifecycleService>();
...
builder.Services.AddScoped<KnotGarden.Features.Ai.GeneratedCredentialFinalizer>();
builder.Services.AddScoped<KnotGarden.Features.Ai.IAiGenerationRunner, KnotGarden.Features.Ai.AiGenerationRunner>();
builder.Services.AddSingleton<KnotGarden.Features.Ai.AiGenerationJobStore>();
builder.Services.AddSingleton<KnotGarden.Features.Ai.AiGenerationQueue>();
builder.Services.AddHostedService<KnotGarden.Features.Ai.AiGenerationWorker>();
...
builder.Services.AddSingleton<KnotGarden.Core.Contracts.IRuntimeArmingState>(sp => sp.GetRequiredService<RuntimeArmingState>());
...
builder.Services.AddSingleton(new KnotGarden.Api.Services.Auth.AuthOptions(authEnabled));
...
using (var scope = app.Services.CreateScope())
{
    ...
    KnotGarden.Api.Services.StartupInitializer.MigrateSchema(db);
    await KnotGarden.Api.Services.StartupInitializer.VerifyAuditChainAsync(db);
    await KnotGarden.Api.Services.StartupInitializer.HealSocketMappingsAsync(db, startupLogger);
