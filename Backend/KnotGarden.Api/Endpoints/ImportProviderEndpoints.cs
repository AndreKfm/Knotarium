...
public static class ImportProviderEndpoints
{
    ...
    {
        app.MapGet("/api/imports/providers", (KnotGarden.NodeRuntime.HostPluginRegistry plugins) =>
        ...
        app.MapPost("/api/imports/{providerId}/preview", async (
            string providerId,
            HttpRequest request,
            KnotGarden.NodeRuntime.HostPluginRegistry plugins) =>
        {
            ...
            var req = new KnotGarden.Core.Contracts.WorkflowImportRequest(
            ...
            AppDbContext db) =>
        {
            ...
            KnotGarden.Core.Contracts.WorkflowImportProviderResult result;
            try
            {
            ...
            foreach (var workflow in result.Workflows)
            {
                ...
                if (existingIds.Contains(toInstall.Id.Value))
                {
                    ...
                    {
                        Id = new KnotGarden.Core.Domain.WorkflowDefinitionId(newId),
    ...
    private static WorkflowExportDocument ToExportDocument(KnotGarden.Core.Domain.WorkflowDefinition def) =>
    ...
    private static IEnumerable<object> ReportRows(KnotGarden.Core.Contracts.WorkflowImportProviderResult r) =>
        r.Report.Entries.Select(e => new { e.Scope, e.Construct, outcome = e.Outcome.ToString(), e.Reason });
    private static IEnumerable<object> ServerRows(KnotGarden.Core.Contracts.WorkflowImportProviderResult r) =>
        r.DiscoveredServers.Select(s => new { s.Alias, s.Host, s.User, s.Enabled });
    private static IEnumerable<object> ProvisionRows(KnotGarden.Core.Contracts.WorkflowImportProviderResult r) =>
