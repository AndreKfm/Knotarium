...
<#
.SYNOPSIS
  Build a productive, copy-and-run build of Knot Garden with the UI included.
...
$root = $PSScriptRoot
$api = Join-Path $root 'Backend/KnotGarden.Api/KnotGarden.Api.csproj'
$frontend = Join-Path $root 'Frontend'
$dist = Join-Path $frontend 'dist'
$appOut = Join-Path $OutputDir 'app'

Write-Host '== Knot Garden — productive build (UI included) ==' -ForegroundColor Cyan
...
$exe = Get-ChildItem $appOut -File |
    Where-Object { $_.BaseName -eq 'KnotGarden.Api' -and ($_.Extension -eq '.exe' -or $_.Extension -eq '') } |
    Select-Object -First 1
$exeName = if ($exe) { $exe.Name } else { 'KnotGarden.Api.exe' }
...
Write-Host 'Note          : the SQLite database (KnotGarden.db) is created next to the exe on first run.'
